    ...
    <div className="grid grid-cols-8 md:grid-cols-10 gap-1">
      {nums.map((n) => {
        ...
        return (
          <div
            key={n}
            className={`aspect-square w-full max-w-[42px] md:max-w-[36px] rounded text-xs font-bold flex items-center justify-center border transition ${tileClass(freq, drawn, flash)}`}
          >
            {n}
          </div>
        );
